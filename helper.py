...
async def download_video(url, cmd, name):
    ...
    # Run command and capture output for better error handling
    process = subprocess.run(download_cmd, shell=True, capture_output=True, text=True)
    ...
    if process.returncode != 0:
        if "visionias" in cmd and failed_counter <= 10:
            ...
            logging.error(f"yt-dlp failed for {url}: {error_msg}")
